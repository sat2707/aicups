...
**Обратите внимание**, данные ниже названия и типы приведены для языка python. Впрочем, для каждого `API` реализованые соответствующие параметры и методы, которые всегда можно посмотреть среди [репозиториев поддерживаемых API](/clients/) (в папках вида `core/api.*`).
...
### `speed` (int)
Скорость движения лифта, измеряется в количестве тиков, которые должны пройти для преодоления лифтом одного этажа. Скорость лифта зависит от количества и веса пассажиров, формулу можно найти ниже в документации
...
Например, если один пассажир весит `1.03`, а второй `1.02`, то скорость лифта следует умножить на вес обоих, то есть `speed` = `50 * 1.03 * 1.02` = `52.53` тиков на этаж вместо изначальных `50`.  
Если лифт везет более `10` пассажиров, то его скорость нужно умножить еще на `1.1`.  
...
При загрузке стратегии из одного файла, имя этого файла не важно (при выборе расширения файла стоит ориентироваться на [базовые стратегии](/baselines/))  
При загрузке стратегии в виде zip-архива, в корне архива должен распологаться файл с тем же именем, что и у [базовых стратегий](/baselines/)
